...
class EntityInsertQueryTest(private val db: Database) {
    ...
    fun createdAt_localDateTime() {
        ...
        val id = db.runQuery { EntityDsl.insert(p, person1) }.personId
        val person2 = db.runQuery { EntityDsl.first(p) { p.personId eq id } }
    ...
    fun createdAt_offsetDateTime() {
        ...
        val id = db.runQuery { EntityDsl.insert(h, human1) }.humanId
        val human2 = db.runQuery { EntityDsl.first(h) { h.humanId eq id } }
    ...
    fun createdAt_customize() {
        ...
        val person1 = Person(1, "ABC")
        val id = myDb.runQuery { EntityDsl.insert(p, person1) }
    ...
    fun uniqueConstraintException() {
        ...
        assertThrows<UniqueConstraintException> {
            db.runQuery { EntityDsl.insert(a, address) }.let { }
        }
    }
    ...
    fun identityGenerator() {
        for (i in 1..201) {
            ...
            val result = db.runQuery { EntityDsl.insert(m, strategy) }
    ...
    fun onDuplicateKeyUpdate_insert() {
        ...
        val query = EntityDsl.insert(d, department).onDuplicateKeyUpdate()
        val count = db.runQuery { query }
        assertEquals(1, count)
        val found = db.runQuery { EntityDsl.first(d) { d.departmentId eq 5 } }
        assertNotNull(found)
    }
    ...
    fun onDuplicateKeyUpdateWithKeys_insert() {
        val d = Department.alias
        val department = Department(5, 50, "PLANNING", "TOKYO", 0)
        val query = EntityDsl.insert(d, department).onDuplicateKeyUpdate(d.departmentNo)
        val count = db.runQuery { query }
        assertEquals(1, count)
        val found = db.runQuery { EntityDsl.first(d) { d.departmentId eq 5 } }
        assertNotNull(found)
    ...
    fun onDuplicateKeyUpdate_update() {
        ...
        val found = db.runQuery { EntityDsl.first(d) { d.departmentId eq 1 } }
    ...
    fun onDuplicateKeyUpdateWithKeys_update() {
        ...
        val found = db.runQuery { EntityDsl.first(d) { d.departmentNo eq 10 } }
    ...
    fun onDuplicateKeyUpdate_update_set() {
        val d = Department.alias
        val department = Department(1, 50, "PLANNING", "TOKYO", 10)
        val query = EntityDsl.insert(d, department).onDuplicateKeyUpdate().set { excluded ->
    ...
    fun onDuplicateKeyUpdateWithKey_update_set() {
        ...
        val query = EntityDsl.insert(d, department)
    ...
    fun onDuplicateKeyIgnore() {
        ...
        val query = EntityDsl.insert(a, address).onDuplicateKeyIgnore()
        val count = db.runQuery { query }
    ...
    fun onDuplicateKeyIgnoreWithKey() {
        ...
        val query = EntityDsl.insert(a, address).onDuplicateKeyIgnore(a.street)
        val count = db.runQuery { query }
        assertEquals(0, count)
    }
