 ...
 * @property updatable whether the column should be updatable (default); if set to `false`,
 * the column is excluded from default update statements generated by `update()` and
 * `insert().onDuplicateKeyUpdate()` DSLs.
 * @property insertable whether the column should be insertable (default); if set to `false`,
 * the column is excluded from default insert statements generated by `insert()` DSL.
 * @property alternateType the alternate type. This type must be a value class.
 * @property length the column length. This property works only in VARCHAR, CHAR, BINARY column type.
 * @property precision the column precision. This property works only in DECIMAL column type.
 * @property scale the column scale. This property works only in DECIMAL column type.
 */
@Target(AnnotationTarget.VALUE_PARAMETER)
@Retention(AnnotationRetention.SOURCE)
