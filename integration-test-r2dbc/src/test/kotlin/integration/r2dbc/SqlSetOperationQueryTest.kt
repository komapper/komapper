...
class SqlSetOperationQueryTest(private val db: R2dbcDatabase) {
    ...
    fun except_entity() = inTransaction(db) {
        val e = Employee.meta
        val q1 = R2dbcSqlDsl.from(e).where { e.employeeId inList listOf(1, 2, 3, 4, 5) }
        val q2 = R2dbcSqlDsl.from(e).where { e.employeeId inList listOf(2, 4, 6, 8) }
    ...
    fun union_entity() = inTransaction(db) {
        val e = Employee.meta
        val q1 = R2dbcSqlDsl.from(e).where { e.employeeId eq 1 }
        val q2 = R2dbcSqlDsl.from(e).where { e.employeeId eq 1 }
        val q3 = R2dbcEntityDsl.from(e).where { e.employeeId eq 5 }
    ...
    fun union_subquery() = inTransaction(db) {
        val e = Employee.meta
        val q1 = R2dbcSqlDsl.from(e).where { e.employeeId eq 1 }.select(e.employeeId)
        val q2 = R2dbcSqlDsl.from(e).where { e.employeeId eq 6 }.select(e.employeeId)
    ...
    fun union_columns() = inTransaction(db) {
        ...
        val q1 =
            R2dbcSqlDsl.from(e).where { e.employeeId eq 1 }
                .select(e.employeeId alias "ID", e.employeeName alias "NAME")
        val q2 = R2dbcSqlDsl.from(a).where { a.addressId eq 2 }
            .select(a.addressId alias "ID", a.street alias "NAME")
        val q3 = R2dbcSqlDsl.from(d).where { d.departmentId eq 3 }
    ...
    fun unionAll_entity() = inTransaction(db) {
        ...
        val q1 = R2dbcSqlDsl.from(e).where { e.employeeId eq 1 }
        val q2 = R2dbcSqlDsl.from(e).where { e.employeeId eq 1 }
        val q3 = R2dbcSqlDsl.from(e).where { e.employeeId eq 5 }
