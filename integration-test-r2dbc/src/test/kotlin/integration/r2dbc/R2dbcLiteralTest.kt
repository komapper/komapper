...
class R2dbcLiteralTest(val db: R2dbcDatabase) {
    @Test
    fun test_literal_boolean(info: TestInfo) = inTransaction(db, info) {
    ...
    @Test
    fun test_literal_boolean_null(info: TestInfo) = inTransaction(db, info) {
        val m = Meta.booleanData
        db.runQuery {
            QueryDsl.insert(m).values {
                m.id eq 1
    ...
    @Test
    fun test_literal_double(info: TestInfo) = inTransaction(db, info) {
    ...
    @Test
    fun test_literal_double_null(info: TestInfo) = inTransaction(db, info) {
        val m = Meta.doubleData
        db.runQuery {
            QueryDsl.insert(m).values {
                m.id eq 1
    ...
    @Test
    fun test_literal_int(info: TestInfo) = inTransaction(db, info) {
        val m = Meta.intData
        db.runQuery {
            QueryDsl.insert(m).values {
                m.id eq 1
    ...
    @Test
    fun test_literal_int_null(info: TestInfo) = inTransaction(db, info) {
        val m = Meta.intData
        db.runQuery {
            QueryDsl.insert(m).values {
                m.id eq 1
    ...
    @Test
    fun test_literal_long(info: TestInfo) = inTransaction(db, info) {
        val m = Meta.longData
        db.runQuery {
            QueryDsl.insert(m).values {
                m.id eq 1
    ...
    @Test
    fun test_literal_long_null(info: TestInfo) = inTransaction(db, info) {
        val m = Meta.longData
        db.runQuery {
            QueryDsl.insert(m).values {
                m.id eq 1
    ...
    @Test
    fun test_literal_offsetDateTime(info: TestInfo) = inTransaction(db, info) {
    ...
    @Test
    fun test_literal_offsetDateTime_illegalStateException(info: TestInfo) = inTransaction(db, info) {
        val value = OffsetDateTime.parse("2025-10-23T12:34:56+09:00")
        val m = Meta.offsetDateTimeData
    ...
    @Run(unless = [Dbms.MYSQL, Dbms.MYSQL_5, Dbms.MARIADB])
    @Test
    fun test_literal_offsetDateTime_null(info: TestInfo) = inTransaction(db, info) {
        val m = Meta.offsetDateTimeData
        db.runQuery {
            QueryDsl.insert(m).values {
                m.id eq 1
    ...
    @Test
    fun test_literal_string(info: TestInfo) = inTransaction(db, info) {
    ...
    @Test
    fun test_literal_string_null(info: TestInfo) = inTransaction(db, info) {
        val m = Meta.stringData
        db.runQuery {
            QueryDsl.insert(m).values {
                m.id eq 1
    ...
    @Test
    fun test_literal_illegal_string(info: TestInfo) = inTransaction(db, info) {
    ...
    @Test
    fun test_literal_enum(info: TestInfo) = inTransaction(db, info) {
        val m = Meta.enumData
        db.runQuery {
            QueryDsl.insert(m).values {
                m.id eq 1
    ...
    @Test
    fun test_literal_enum_null(info: TestInfo) = inTransaction(db, info) {
        val m = Meta.enumData
        db.runQuery {
            QueryDsl.insert(m).values {
                m.id eq 1
    ...
    @Test
    fun test_literal_userDefinedInt(info: TestInfo) = inTransaction(db, info) {
    ...
    @Test
    fun test_literal_userDefinedInt_null(info: TestInfo) = inTransaction(db, info) {
        val m = Meta.userDefinedIntData
        db.runQuery {
            QueryDsl.insert(m).values {
                m.id eq 1
    ...
    @Test
    fun test_literal_userDefinedDouble(info: TestInfo) = inTransaction(db, info) {
    ...
    @Run(onlyIf = [Dbms.POSTGRESQL])
    @Test
    fun test_literal_userDefinedDouble_null(info: TestInfo) = inTransaction(db, info) {
        val m = Meta.userDefinedDoubleData
        db.runQuery {
            QueryDsl.insert(m).values {
                m.id eq 1
    ...
    @Test
    fun test_nullLiteral_string_null(info: TestInfo) = inTransaction(db, info) {
        val m = Meta.stringData
        db.runQuery {
            QueryDsl.insert(m).values {
                m.id eq 1
    ...
    @Test
    fun test_nullLiteral_enum_null(info: TestInfo) = inTransaction(db, info) {
        val m = Meta.enumData
        db.runQuery {
            QueryDsl.insert(m).values {
                m.id eq 1
