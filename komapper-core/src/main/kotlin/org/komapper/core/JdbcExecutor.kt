...
class JdbcExecutor(
    ...
    private val autoGeneratedKeys: Int = if (assignment is Assignment.Identity<*, *>) {
    ...
    fun executeUpdate(statement: Statement): Pair<Int, LongArray> {
        return executeWithExceptionCheck {
            config.session.connection.use { con ->
                log(statement)
                con.prepareStatement(statement.sql, autoGeneratedKeys).use { ps ->
                    ps.setUp()
                    ps.bind(statement.values)
    ...
    fun executeBatch(statements: List<Statement>): Pair<IntArray, LongArray> {
        ...
        return executeWithExceptionCheck {
            config.session.connection.use { con ->
                ...
                log(firstStatement)
                con.prepareStatement(firstStatement.sql, autoGeneratedKeys).use { ps ->
                    ps.setUp()
    ...
    private fun PreparedStatement.fetchGeneratedKeys(): LongArray {
        return if (autoGeneratedKeys == java.sql.Statement.RETURN_GENERATED_KEYS) {
